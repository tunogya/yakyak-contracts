...
*/
abstract contract RewardsPool is IRewardsPool, Ownable, ReentrancyGuard, IERC721Receiver {
  ...
  modifier canAddLiquidity(uint256 _amount) {
    require(_canAddLiquidity(_amount), "RewardsPool/exceeds-liquidity-cap");
    _;
  }
  ...
  /// @inheritdoc IRewardsPool
  function isControlled(IPass _controlledToken) external view override returns (bool) {
  ...
  function award(address _to, uint256 _amount) external override onlyPrizeStrategy {
    ...
    require(_amount <= currentAwardBalance, "RewardsPool/award-exceeds-avail");
  ...
  function awardExternalERC721(
    ...
    require(_canAwardExternal(_externalToken), "RewardsPool/invalid-external-token");
